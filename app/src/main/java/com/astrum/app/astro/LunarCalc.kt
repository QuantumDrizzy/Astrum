...
object LunarCalc {
    ...
    fun phaseName(phase: Double): String {
        val p = ((phase % 1.0) + 1.0) % 1.0
        return when {
            p < 0.03 || p > 0.97 -> "Luna Nueva"
            p < 0.22 -> "Luna Creciente"
            p < 0.28 -> "Cuarto Creciente"
            p < 0.47 -> "Gibosa Creciente"
            p < 0.53 -> "Luna Llena"
            p < 0.72 -> "Gibosa Menguante"
            p < 0.78 -> "Cuarto Menguante"
            else -> "Luna Menguante"
        }
    }
}
