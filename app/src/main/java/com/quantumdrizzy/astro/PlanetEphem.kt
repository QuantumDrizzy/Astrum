 ...
 *
 * This is the reconciled superset of the two app copies (Skytime tracked 4 planets with
 * altAz/altitude/elongation; Astrum needed 7 with equatorial/helioDist for its PlanetData). The
 * shared spine carries all 7 planets + Earth and the full API; each app filters BRIGHT to taste.
 *
 * Cross-checked against published planet visibility for June 2026: Venus evening/west, Jupiter low
 * evening, Saturn morning/east, Mars low pre-dawn.
 *
...
object PlanetEphem {
    ...
    // symbol, a(AU)+rate, e+rate, I(deg)+rate, L(deg)+rate, ϖ(deg)+rate, Ω(deg)+rate (rates/century)
    private class El(
        val symbol: String,
    ...
    private val E = linkedMapOf(
        "Mercury" to El("☿", 0.38709927, 0.00000037, 0.20563593, 0.00001906, 7.00497902, -0.00594749,
            252.25032350, 149472.67411175, 77.45779628, 0.16047689, 48.33076593, -0.12534081),
        "Venus" to El("♀", 0.72333566, 0.00000390, 0.00677672, -0.00004107, 3.39467605, -0.00078890,
            181.97909950, 58517.81538729, 131.60246718, 0.00268329, 76.67984255, -0.27769418),
        "Mars" to El("♂", 1.52371034, 0.00001847, 0.09339410, 0.00007882, 1.84969142, -0.00813131,
            -4.55343205, 19140.30268499, -23.94362959, 0.44441088, 49.55953891, -0.29257343),
        "Jupiter" to El("♃", 5.20288700, -0.00011607, 0.04838624, -0.00013253, 1.30439695, -0.00183714,
            34.39644051, 3034.74612775, 14.72847983, 0.21252668, 100.47390909, 0.20469106),
        "Saturn" to El("♄", 9.53667594, -0.00125060, 0.05386179, -0.00050991, 2.48599187, 0.00193609,
            49.95424423, 1222.49362201, 92.59887831, -0.41897216, 113.66242448, -0.28867794),
        "Uranus" to El("⛢", 19.18916464, -0.00196176, 0.04725744, -0.00004397, 0.77263783, -0.00242939,
            313.23810451, 428.48202785, 170.95427630, 0.40805281, 74.01692503, 0.04240589),
        "Neptune" to El("♆", 30.06992276, 0.00026291, 0.00859048, 0.00005105, 1.77004347, 0.00035372,
            -55.12002969, 218.45945325, 44.96476227, -0.32241464, 131.78422574, -0.00508664),
    )

    /** All planets the spine knows. Apps filter to the subset they display. */
    val ALL: List<String> = E.keys.toList()

    /** Naked-eye planets that are easy evening/morning targets (Skytime's working set). */
    val BRIGHT: List<String> = listOf("Venus", "Mars", "Jupiter", "Saturn")

    fun symbol(name: String): String = E[name]?.symbol ?: "•"
    ...
    /** Altitude and azimuth (deg) of `name` at `date` from (lat,lng). */
    fun altAz(name: String, date: Date, latDeg: Double, lngDeg: Double): Pair<Double, Double> {
        val eq = equatorial(name, date) ?: return -90.0 to 0.0
        val h = AstroEngine.equatorialToHorizontal(eq.first, eq.second, date, latDeg, lngDeg)
        return h.altitude to h.azimuth
    }

    fun altitude(name: String, date: Date, latDeg: Double, lngDeg: Double): Double =
        altAz(name, date, latDeg, lngDeg).first

    /** Angular distance from the Sun (deg) — near 180° at opposition (outer planets). */
    fun elongation(name: String, date: Date): Double {
        val g = geo(name, date) ?: return 0.0
        val px = g[0]; val py = g[1]; val pz = g[2]
        val sx = -g[3]; val sy = -g[4]; val sz = -g[5]  // Sun direction from Earth = −Earth_helio
        val dot = px * sx + py * sy + pz * sz
        val mp = sqrt(px * px + py * py + pz * pz)
        val ms = sqrt(sx * sx + sy * sy + sz * sz)
        return acos((dot / (mp * ms)).coerceIn(-1.0, 1.0)) * DEG
    }
}
